...
# Toggle to create new hogMatrix for each Pokemon, otherwise use the loads instead and comment these out
...
dump(hogMatrix0, hogFolderPath + labelVector[0] + 'HOG_PCA_Matrix.joblib')
dump(hogMatrix1, hogFolderPath + labelVector[1] + 'HOG_PCA_Matrix.joblib')
dump(hogMatrix2, hogFolderPath + labelVector[2] + 'HOG_PCA_Matrix.joblib')
dump(hogMatrix3, hogFolderPath + labelVector[3] + 'HOG_PCA_Matrix.joblib')

'''
hogMatrix0 = load(labelVector[0] + 'HOG_PCA_Matrix.joblib')
hogMatrix1 = load(labelVector[1] + 'HOG_PCA_Matrix.joblib')
hogMatrix2 = load(labelVector[2] + 'HOG_PCA_Matrix.joblib')
hogMatrix3 = load(labelVector[3] + 'HOG_PCA_Matrix.joblib')
'''
...
# Toggle to create SVMs from scratch, but then comment out the SVM loads if you do
...
#print(svm0.support_vectors_.shape)
#print(svm1.support_vectors_.shape)
#print(svm2.support_vectors_.shape)
#print(svm3.support_vectors_.shape)
...
dump(svm0, svmFolderPath + 'pikachuSVM.joblib')
dump(svm1, svmFolderPath + 'bulbasaurSVM.joblib')
dump(svm2, svmFolderPath + 'charmanderSVM.joblib')
dump(svm3, svmFolderPath + 'squirtleSVM.joblib')

# Toggle to how the SVMs compare to the training set
'''
print('Testing the SVMs on the Training Set...')
numOfTestingData = len(hogMatrix0[0:240])
pikaRight = 0
bulbaRight = 0
charRight = 0
squirtRight = 0
...
print(labelVector[0])
pikachuPredictions = svm0.predict(hogMatrix0[0:240])
pikachuScores = svm0.score_samples(hogMatrix0[0:240])
for i in range(numOfTestingData):
  #print(i+1, pikachuPredictions[i], pikachuScores[i])
  if(pikachuPredictions[i] == 1):
    pikaRight = pikaRight + 1
print('{0}% accurate on training data'.format(pikaRight/numOfTestingData))

print(labelVector[1])
bulbasaurPredictions = svm1.predict(hogMatrix1[0:240])
bulbasaurScores = svm1.score_samples(hogMatrix1[0:240])
for i in range(numOfTestingData):
  #print(i+1, bulbasaurPredictions[i], bulbasaurScores[i])
  if(bulbasaurPredictions[i] == 1):
    bulbaRight = bulbaRight + 1
print('{0}% accurate on training data'.format(bulbaRight/numOfTestingData))

print(labelVector[2])
charmanderPredictions = svm2.predict(hogMatrix2[0:240])
charmanderScores = svm2.score_samples(hogMatrix2[0:240])
for i in range(numOfTestingData):
  #print(i+1, charmanderPredictions[i], charmanderScores[i])
  if(charmanderPredictions[i] == 1):
    charRight = charRight + 1
print('{0}% accurate on training data'.format(charRight/numOfTestingData))

print(labelVector[3])
squirtlePredictions = svm3.predict(hogMatrix3[0:240])
squirtleScores = svm3.score_samples(hogMatrix3[0:240])
for i in range(numOfTestingData):
  #print(i+1, squirtlePredictions[i], squirtleScores[i])
  if(squirtlePredictions[i] == 1):
    squirtRight = squirtRight + 1
print('{0}% accurate on training data'.format(squirtRight/numOfTestingData))
'''
# Toggle to how the SVMs compare to the testing set
# Testing the SVMs and viewing their results
'''
print('Testing the SVMs on the Testing Set...')
...
'''
for each testing img
  add the correct label into vector of true labels
  predict the Pokemon
  add the prediction into a vector of predictions
...
def predictPokemon(testImgURL):
  ...
  #print(predictions) #Toggle to see the predictions per image tested
...
create a confusion matrix
'''
# Toggle to test each image in testing data and get a confusion matrix
trueLabels = np.concatenate((np.full((60), labelVector[0]), np.full((60), labelVector[1]), np.full((60), labelVector[2]), np.full((60), labelVector[3])))
predictions = np.empty_like(trueLabels)
index = 0
for i in range(4):

  if(i == 0):
    dataSet = pikachuTestingData
  elif(i==1):
    dataSet = bulbasaurTestingData
  elif(i==2):
    dataSet = charmanderTestingData
  elif(i==3):
    dataSet = squirtleTestingData
  for k in range(dataSet.shape[0]):
    predictions[index] = predictPokemon(dataSet[k])
    index = index + 1

confusion = sklearn.metrics.confusion_matrix(trueLabels, predictions, labelVector)
print('Confusion Matrix')
print(confusion)

print('Training the Pokedex SVMs is complete!')